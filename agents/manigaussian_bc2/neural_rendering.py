...
class NeuralRenderer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.use_CEloss = 21 # 0 #21 # 7 #21 # 常用：0/7/1  2:两个维度 7：3维度-ignore0 5搞笑版language代替mask 6 onlyembed
    ...
    def L1_loss_bg(self,pred, target): # 1.0gpt?
        """
        L1 处理one hot
        Args:
            pred: (B, C, H, W) 预测的logits
            target: (B, H, W) 目标掩码，值为0到C-1的长整型
        """
        # pred = F.softmax(pred, dim=1)
        # pred = pred/2 +0.5 # 前面运行的没有这句
        target = F.one_hot(target, num_classes=pred.shape[1]).permute(0, 3, 1, 2)  # (B, C, H, W)
        total_loss = l1_loss(pred, target)    
        # print(f"L1 total_loss = {total_loss};")
        return total_loss

    def dice_L1_loss(self,pred, target, smooth=1e-8): # 1.0gpt?
        """
        Args:
            pred: (B, C, H, W) 预测的logits
            target: (B, H, W) 目标掩码，值为0到C-1的长整型
        ...
        # print(f"total_loss = {total_loss}; dice_loss = {dice_loss}; loss_l1 = {loss_l1}")
        return total_loss

    def dice_loss_bg(self,pred, target, smooth=1e-8): # 1.0gpt?
        """
        基础的Dice Loss 均有bg(但是输出是黑色+一点点红色)
        Args:
            pred: (B, C, H, W) 预测的logits
            target: (B, H, W) 目标掩码，值为0到C-1的长整型
            smooth: 平滑项，防止分母为0
        ...
        # print(f"total_loss = {total_loss}; dice_loss = {dice_loss}; ce_loss = {ce_loss}")
        return total_loss

    def dice_loss_ignorebg1(self,pred, target, smooth=1e-8): # 1.0gpt?
        """
        Dice Loss ： 不忽略背景    CE-log? ： 忽略 
        Args:
            pred: (B, C, H, W) 预测的logits
            target: (B, H, W) 目标掩码，值为0到C-1的长整型
            smooth: 平滑项，防止分母为0
    ...
    def dice_loss(self,pred, target, smooth=1e-8): # 1.0gpt?
        ...
        # 将预测和目标展平     pred_flat = pred_foreground.reshape(-1)  # (B*(C-1)*H*W,)     target_flat = target_foreground.reshape(-1)  # (B*(C-1)*H*W,)
        # 计算交集intersection = torch.sum(pred_flat * target_flat)
        intersection = torch.sum(pred_foreground * target_foreground)
        # 计算并集      pred_sum = torch.sum(pred_flat)     target_sum = torch.sum(target_flat)
    ...
    def focal_loss(self,pred, target, smooth=1e-8,alpha=0.25, gamma=2.0):
        ...
        pt = (pred * target).sum(1)  # 取出对应类别的概率
        ...
        # print(f"total_loss = {total_loss}; dice_loss = {dice_loss}; focal_loss = {focal_loss}")
        return total_loss
    ...
    def ce_weight_ignore_l1(self, pred, target):
        ...
        weights = weights.to(device)
        celoss = nn.CrossEntropyLoss(weight=weights, ignore_index=0)
        # print("pred",pred.shape,"target",target.shape,weights.shape)
        ce_loss = celoss(pred, target)
        target = F.one_hot(target, num_classes=pred.shape[1]).permute(0, 3, 1, 2)  # (B, C, H, W)
        # pred = pred/2 +0.5
        loss_l1 = l1_loss(pred, target)  
        # loss_l1test = l1_loss(target1.float(), target1)         # Test测试[target, target]的loss
        # loss_cetest = celoss(target1.float(), target)
        # total_loss_test = loss_l1test * 0.4 + loss_cetest * 0.6
        # total_loss = loss_l1 * 0.1 + ce_loss * 0.9
        total_loss = loss_l1 * 0.5 + ce_loss * 0.5
        # total_loss = loss_l1 * 0.7 + ce_loss * 0.3
        # print(f"total_loss = {total_loss}; loss_l1 = {loss_l1}; ce_loss = {ce_loss}") 
        # print(f"total_loss_test = {total_loss_test}; loss_l1test = {loss_l1test}; loss_cetest = {loss_cetest}") # 0.22 0 0.55
        return total_loss
    ...
    def ce_weight_ignore_l1_nosoft(self, pred, target, smooth=1e-8):
        ...
        # print("0 pred",pred.shape,pred)
        # print("1 pred",pred.shape,pred)
        pred1 = torch.log(pred)
        # print("2 pred1",pred1.shape) # [1, 3, 256, 256]
        # print("3 target", target.shape) # [1, 128, 128]
        ce_loss = celoss(pred1, target)

        target = F.one_hot(target, num_classes=pred.shape[1]).permute(0, 3, 1, 2)  # (B, C, H, W)
        # print(pred.shape[1])
        # loss_l1test = l1_loss(target1.float(), target1)
        loss_l1 = l1_loss(pred, target)  
        # target1 = torch.log(target1+smooth)
        # loss_cetest = celoss(target1.float(), target)
        # total_loss_test = loss_l1test * 0.1 + loss_cetest * 0.9
        total_loss = loss_l1 * 0.7 + ce_loss * 0.3 
        # print(f"total_loss = {total_loss}; loss_l1 = {loss_l1}; ce_loss_nosoft = {ce_loss}")
        # print(f"total_loss_test = {total_loss_test}; loss_l1test = {loss_l1test}; loss_cetest = {loss_cetest}") # 0 0 0
        return total_loss
    ...
    def dice_loss_bg_weight(self,pred, target, smooth=1e-8): # 1.0gpt?
        """
        基础的Dice Loss 均有bg(但是输出是黑色+一点点红色)
            pred: (B, C, H, W) 预测的logits
            target: (B, H, W) 目标掩码，值为0到C-1的长整型
    ...
    def _mask_loss_fn(self, render_mask, gt_mask):
        ...
        if self.use_CEloss == 1:
            ...
            # 3 focal+ dice 但是好像有问题
            # loss = self.focal_loss(render_mask, gt_mask)
            # 4 L1 + one hot但是 100 010 001 蓝色 + 黑色的mask不知道会不会变好
            # loss = self.L1_loss_bg(render_mask, gt_mask)
            # 5 Ldice不忽略背景 Lce-soft忽略 上升
            # loss = self.dice_loss_ignorebg1(render_mask, gt_mask)
            # 6 dice+L1 mask得到纯黑色
            # loss = self.dice_L1_loss(render_mask, gt_mask)
            # 7 ce_weight_ignore 带权重 ignore的简单ce             loss 1.06左右不变 大块红色周围有点蓝色
            # loss = self.ce_weight_ignore(render_mask, gt_mask)
            # 8 ce_weight_ignore_l1 带权重 ignore的简单ce + L1     0.3:0.7 0.3*1.3+0.4*0.7  loss下降，有红色，看起来还行 
            # loss = self.ce_weight_ignore_l1(render_mask, gt_mask)
            # 9 ce_weight_ignore_l1_nosoft 0.5:0.5Nan了，但是当时的颜色蓝色大块在上面
            loss = self.ce_weight_ignore_l1_nosoft(render_mask, gt_mask)
            # 10 ce_weight_l1_nosoft       0.5:0.5 蓝色很少，Loss上升 
    ...
    def encode_data(self, pcd, dec_fts, lang, 
        ...
        data['xyz'] = einops.rearrange(pcd, 'b c h w -> b (h w) c') # bs,256*256,(xyz)        #  einops.rearrange 函数重新排列点云数据 pcd   'b c h w -> b (h w) c' 是一个重组操作的模式，它将输入张量从四维格式（可能表示 [batch_size, channels, height, width]）转换为三维格式，  其中高度和宽度被合并为一个维度。这通常用于将3D点云数据从图像格式转换为列表格式，
        ...
        if self.use_dynamic_field:
            if self.field_type =='bimanual':      # 双臂同时工作 ManiGaussian*2
                data['next'] = {
                    'extr': next_tgt_pose,
            ...
            elif self.field_type =='LF':            # leader follower 模式 
    ...
    def forward(self, pcd, dec_fts, language, mask=None, gt_rgb=None, gt_pose=None, gt_intrinsic=None, rgb=None, depth=None, camera_intrinsics=None, camera_extrinsics=None, 
        ...
        if self.mask_gen == 'pre':
            ...
            if self.use_dynamic_field:
                ...
                if self.use_CEloss == 0:
                    ...
                    gt_mask_label = self.mask_label_onehot(gt_mask) # 默认7
                if self.use_dynamic_field:
                    next_gt_rgb = next_gt_rgb.permute(0,2,3,1)
                    next_gt_mask = next_gt_mask.permute(0,2,3,1)
                    next_gt_mask_label = self.mask_label_onehot(next_gt_mask) # 默认7
        ...
        if training:
            ...
            # print("render_novel",render_novel.shape)  
            ...
            if self.mask_gen == 'nonerf':
                ...
                if True:
                    ...
                    # data =self.pts2render_mask_gen_detach(data, bg_color=self.bg_mask)
                ...
                if not self.use_dynamic_field :
                    loss_dyna_mask = loss_dyna_mask_novel
                    lambda_mask =1   #if step >= 1000 else 0 # 3000
                ...
                # print("----------------------", self.use_CEloss)
                if self.use_CEloss==1 or self.use_CEloss == 7 or self.use_CEloss == 21:
                    render_mask_novel = data['novel_view']['mask_pred']
                    # print("render_mask_novel", render_mask_novel.shape) # [1, 3, 256, 256]
                    # print("gt_mask_label", gt_mask_label.shape) # [1, 128, 128]
                    loss_dyna_mask_novel = self._mask_loss_fn(render_mask_novel, gt_mask_label) #gt_mask) # mask现阶段的 _mask_loss_fn
                    render_mask_novel = render_mask_novel.permute(0, 2, 3, 1)
                ...
                # print("1 gt_mask_label = ",gt_mask_label.shape)              # [1 128 128]
                # print("2 render_mask_novel = ",render_mask_novel.shape)  # [1 3 128 128]
            ...
            if self.field_type == 'bimanual':
                ...
                if self.mask_gen == 'gt':   # mask现成的gt
                    ...
                    if self.use_dynamic_field and (next_gt_rgb is not None and gt_mask is not None):
                        ...
                        next_loss_dyna_mask_left = torch.tensor(0.) #self._mask_loss_fn(next_render_mask, next_gt_mask_label) # mask去左臂的mask
    ...
    def pts2render(self, data: dict, bg_color=[0,0,0]):
        ...
        # .unsqueeze(0): 在张量的第0个维度（即最前面）增加一个维度。
    ...
    def pts2render1(self, data: dict, bg_color=[0,0,0]):
        '''(用MASK) feature_language_i用mask赋值'''
        bs = data['intr'].shape[0]
        assert bs == 1, "batch size should be 1"
        i = 0
    ...
    def createby_gt_mask(self, data: dict, gt_mask=None, gt_mask_camera_extrinsic=None, gt_mask_camera_intrinsic=None, next_gt_mask = None,
        ...
        mask_3d = None    # now注释了 不需要训练有用的只有去掉next的用法
        # # --------上面部分是now阶段的写法，耗时久，注释了------以下next--------------------------------------------------------------------------------------
        # time1 = time.perf_counter()
        # print('1 time', time1)
    ...
    def generate_final_class_labels_ce1(self,next_left_mask_gen):
        """
        去除背景loss计算的(label-1) 排除左手区域。 1 128 128 3 -> 1 128 128 2 -> output: 1 128 128 [T:1 / F:0]
        """
        # exclude_left_mask = (next_left_mask_gen > 2.5) | (next_left_mask_gen < 1.5) # 原来 值的方式来判断
    ...
    def mask_label(self,gt_mask):
        ...
        gt_mask_label[(gt_mask > right_min-1) & (gt_mask < right_max+1)] = 0.0 #1.0
        gt_mask_label[(gt_mask > left_min-1) & (gt_mask < left_max+1)] = 1.0 #2.0
    ...
    def vis_labels_ce1(self,next_left_mask_gen):
        """
        (vis_labels - bg | 减去bgloss计算) vis [1 128 128 3] -> [1 128 128 2] - > rgb
        """
        # 获取每个像素的最大类索引
        next_left_mask_gen = next_left_mask_gen[:,:,:,[0,1]]
        class_indices = torch.argmax(next_left_mask_gen, dim=-1)
    ...
    def vis_labelsL1(self,mask):
        ...
        """
        mask_mean = mask.mean(dim=-1)   # 取average
        # mask_mean = mask[:,:,:,0]     # 取第一维    
        ...
        # [-1, 1]
        mask_rgb[(mask_mean >= -0.4) & (mask_mean < 0.4)] = torch.tensor([255, 0, 0], dtype=torch.uint8) # 红色
