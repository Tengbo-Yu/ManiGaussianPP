...
def getWorld2View2(R, t, translate=np.array([.0, .0, .0]), scale=1.0):
    # 将相机的世界坐标转换为视图坐标 
    # 生成了一个从世界坐标系到相机视图坐标系的变换矩阵（view matrix），并通过缩放和平移相机中心点来调整视角
    # 接受旋转矩阵R、平移向量t、平移偏移translate（默认为零向量）和缩放因子scale（默认为1.0）。该函数计算从世界坐标到观察坐标的变换矩阵。
    """R: 3x3 的旋转矩阵，用于表示相机的旋转。
    t: 3x1 的平移向量，用于表示相机的平移。
    translate: 一个 3D 向量（默认为 [0,0,0]），用于对相机中心进行平移调整。
    scale: 一个标量值（默认为 1.0），用于对相机中心进行缩放调整。"""
    Rt = np.zeros((4, 4))
    Rt[:3, :3] = R.transpose()
    Rt[:3, 3] = t
    Rt[3, 3] = 1.0
