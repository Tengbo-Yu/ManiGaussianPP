...
replay_path="/data1/zjyang/program/peract_bimanual/replay/debug/overfit"
...
# for debug
use_dynamic_field=False # False # True #False
apply_se3=False
render_freq=50 #100 #2000
...
# 下面那个里面pre是 自己训练的分割左手的方法； GT是用MASK+depth图片去投影到三维区域去除左臂的（相当于减去了训练MASK的步骤）；  None好像是直接不去掉了
