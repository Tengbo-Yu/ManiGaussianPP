...
# Mani----------------------------------------------------
def perturb_se3_camera_pose(camera_pose,        # 原始的相机姿态矩阵列表，每个矩阵是 [bs, 4, 4] 尺寸。
                trans_shift_4x4,                # 平移变换矩阵，尺寸为 [bs, 4, 4]。    
                rot_shift_4x4,                  # 旋转变换矩阵，尺寸为 [bs, 4, 4]。
                action_gripper_4x4,             # 动作抓取器的姿态矩阵，尺寸为 [bs, 4, 4]。
                bounds):                        # 场景的度量边界，尺寸为 [bs, 6]。
    """ 
    !! 对相机姿态进行扰动处理，以模拟不同的视角变化。
    Perturb point clouds with given transformation.
    :param pcd: list of point clouds [[bs, 3, N], ...] for N cameras
    :param trans_shift_4x4: translation matrix [bs, 4, 4]
    :param rot_shift_4x4: rotation matrix [bs, 4, 4]
    ...
    for cam_pose in camera_pose:
        ...
        # 计算扰动前的平移向量
        # 从 action_gripper_4x4 中提取平移向量，并将其从当前相机的平移向量 cam_T 中减去
        action_trans_3x1 = action_gripper_4x4[:, 0:3, 3].unsqueeze(-1)
        trans_shift_3x1 = trans_shift_4x4[:, 0:3, 3].unsqueeze(-1)

        # (应用旋转扰动)将 cam_T 扩展为 4x1 矩阵 cam_T_4x1，然后应用旋转扰动 rot_shift_4x4
        ...
        # apply bounded translations 应用有界平移
        # 根据 bounds 计算 x、y、z 方向上的最小值和最大值。
        bounds_x_min, bounds_x_max = bounds[:, 0].min(), bounds[:, 3].max()
        bounds_y_min, bounds_y_max = bounds[:, 1].min(), bounds[:, 4].max()
        bounds_z_min, bounds_z_max = bounds[:, 2].min(), bounds[:, 5].max()

        # 对 action_trans_3x1 和 trans_shift_3x1 进行边界限制 
        action_then_trans_3x1 = action_trans_3x1 + trans_shift_3x1
        ...
        # get new camera pose 更新相机姿态
        cam_T = cam_T_4x1[:, :3, :]
        # 将扰动后的旋转矩阵 cam_R 和平移向量 cam_T 组合成新的相机姿态
